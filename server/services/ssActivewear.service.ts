...
export class SsActivewearService {
  ...
  async importProducts(userId: string, styleFilter?: string): Promise<string> {
    // Create import job
    const importJob = await integrationRepository.createSsActivewearImportJob({
      userId,
      status: 'pending',
    });
    ...
    this.processImport(importJob.id, styleFilter).catch(error => {
      ...
      integrationRepository.updateSsActivewearImportJob(importJob.id, {
        status: 'failed',
  ...
  private async processImport(jobId: string, styleFilter?: string): Promise<void> {
    await integrationRepository.updateSsActivewearImportJob(jobId, {
      status: 'running',
      startedAt: new Date(),
    });
    ...
    try {
      ...
      for (const product of products) {
        try {
          const existingProduct = await integrationRepository.getSsActivewearProductBySku(product.sku);
          ...
          if (existingProduct) {
            await integrationRepository.updateSsActivewearProduct(existingProduct.id, productData);
            updatedProducts++;
          } else {
            await integrationRepository.createSsActivewearProduct(productData);
            newProducts++;
          }
