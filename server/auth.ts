...
export async function setupAuth(app: Express) {
  ...
  app.post("/api/auth/2fa/verify", async (req, res) => {
    try {
      ...
      if (!isValid && user.twoFactorBackupCodes) {
        ...
        if (result.valid) {
          isValid = true;
          await storage.setTwoFactorBackupCodes(user.id, result.updatedCodes);
        }
      }
  ...
  app.post("/api/auth/2fa/setup", isAuthenticated, async (req: any, res) => {
    try {
      ...
      await storage.setTwoFactorSecret(userId, encrypted);
  ...
  app.post("/api/auth/2fa/verify-setup", isAuthenticated, async (req: any, res) => {
    try {
      ...
      await storage.setTwoFactorBackupCodes(userId, hashedCodes);
