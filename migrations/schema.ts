import { pgTable, foreignKey, varchar, text, jsonb, timestamp, boolean, integer, numeric, unique, index, pgEnum } from "drizzle-orm/pg-core"
...
export const productionStages = pgTable("production_stages", {
	...
	createdAt: timestamp("created_at", { mode: 'string' }).defaultNow(),
	updatedAt: timestamp("updated_at", { mode: 'string' }).defaultNow(),
	icon: varchar().default('Package').notNull(),
});
