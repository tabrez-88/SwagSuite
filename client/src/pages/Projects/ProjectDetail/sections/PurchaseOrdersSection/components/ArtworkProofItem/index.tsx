...
import {
  CheckCircle, ExternalLink, Eye, Loader2, Mail,
  Palette, ShieldCheck, Upload,
} from "lucide-react";
...
export interface ArtworkProofItemProps {
  ...
  isLocked: boolean;
  onUpdateArtwork: (params: { artworkId: string; orderItemId: string; updates: Record<string, unknown> }) => void;
  isUpdating: boolean;
...
export default function ArtworkProofItem({
  artwork: art, isLocked, onUpdateArtwork, isUpdating,
  onUploadProof, onOpenApprovalLink, onPreviewFile,
      ...
      <div className="flex items-center gap-3 p-3">
        <div
          ...
          onClick={() => {
            ...
            if (url) onPreviewFile({ url, name: (art.name as string) || "Artwork" });
          }}
        >
        ...
        <div className="px-3 pb-2">
          <div className="flex items-center gap-3 p-2 bg-blue-50 rounded-md border border-blue-100">
            <div className="w-10 h-10 flex-shrink-0 bg-white rounded border overflow-hidden cursor-pointer"
              onClick={() => onPreviewFile({ url: art.proofFilePath as string, name: (art.proofFileName as string) || "Vendor Proof" })}>
              {(() => {
        ...
        <div className="flex items-center gap-2 px-3 pb-3 flex-wrap">
          {canRequestProof && (
            <Button variant="outline" size="sm" className="h-7 text-xs gap-1"
              ...
              {isUpdating ? <Loader2 className="w-3 h-3 animate-spin" /> : <Mail className="w-3 h-3" />} Request Proof
            </Button>
          )}
          ...
          {canMarkComplete && (
            <Button variant="outline" size="sm" className="h-7 text-xs gap-1 border-green-200 text-green-700 hover:bg-green-50"
              ...
              {isUpdating ? <Loader2 className="w-3 h-3 animate-spin" /> : <CheckCircle className="w-3 h-3" />} Complete
            </Button>
          )}
