    ...
    <div className="min-h-screen bg-gray-100" style={{ fontFamily: font }}>
      ...
      {selectedProduct && (
        <ProductLightbox
          ...
          onCommentPosted={() => queryClient.invalidateQueries({ queryKey: [`/api/presentation/${token}`] })}
        />
      )}
    </div>
  );
...
function ProductLightbox({
  item, items, presentation, comments, token, onClose, onNavigate, onCommentPosted,
}: {
  item: any;
  items: any[];
