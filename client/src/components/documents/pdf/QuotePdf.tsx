/**
 * Quote PDF — react-pdf rewrite of QuoteTemplate.tsx.
 *
 * Renders a true vector PDF (selectable text, crisp at any zoom) using
 * @react-pdf/renderer. Each product item is wrapped in `<View wrap={false}>`
 * so a long quote breaks across pages WITHOUT splitting an item table or
 * its artwork details. Header / footer use the `fixed` prop so they repeat
...
export function QuotePdf({
  ...
  // Subtotal — same shared pricing util as the rest of the app.
    ...
    <Document title={`Quote ${order?.orderNumber || ""}`}>
      <Page size="A4" style={styles.page} wrap>
          ...
          <View style={styles.notesBlock} wrap={false}>
            {order?.notes && (
              <>
                ...
                <Text style={[styles.notesText, { marginBottom: 8 }]}>{order.notes}</Text>
              </>
            )}
            {order?.additionalInformation && (
              <>
                <Text style={styles.notesLabel}>ADDITIONAL INFORMATION:</Text>
                <Text style={styles.notesText}>{order.additionalInformation}</Text>
              </>
            )}
          </View>
        )}
        ...
        <View style={styles.pageFooter} fixed>
          ...
          <Text style={{ marginTop: 2 }}>This quote is valid for 30 days from the date issued.</Text>
        </View>
        <Text
