...
import { Shirt, HardHat, Package, Lightbulb, Star, Calendar, ExternalLink, Search, Bot, Building2 } from 'lucide-react';
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/components/ui/select';
import type { PopularProduct, SuggestedProduct } from './types';
import { usePopularProducts } from './hooks';

function ProductCard({ product, index, onClick }: { product: PopularProduct; index: number; onClick: (p: PopularProduct) => void }) {
  return (
    <div
      key={product.id}
...
function SuggestedCard({ product, index, onNavigate }: { product: SuggestedProduct; index: number; onNavigate: (name: string) => void }) {
  return (
        ...
        <div className="space-y-6">
            ...
            <Card>
              ...
              <CardContent>
                {isLoading ? <LoadingSkeleton /> : (
                  <div className="space-y-4 h-[240px] overflow-y-auto scrollbar-thin scrollbar-thumb-gray-300 hover:scrollbar-thumb-gray-400">
                    {products
                      .filter((p: any) => p.productType === 'apparel')
                      .map((product: any, index: number) => (
                        <ProductCard key={product.id} product={product} index={index} onClick={handleProductClick} />
                      ))}
                    {products.filter((p: any) => p.productType === 'apparel').length === 0 && (
                      <EmptyState icon={Package} message="No apparel items found for this period" />
                    )}
                  </div>
                )}
              </CardContent>
            ...
            <Card>
              ...
              <CardContent>
                {isLoading ? <LoadingSkeleton /> : (
                  <div className="space-y-4 h-[240px] overflow-y-auto scrollbar-thin scrollbar-thumb-gray-300 hover:scrollbar-thumb-gray-400">
                    {products
                      .filter((p: any) => p.productType === 'hard_goods')
                      .map((product: any, index: number) => (
                        <ProductCard key={product.id} product={product} index={index} onClick={handleProductClick} />
                      ))}
                    {products.filter((p: any) => p.productType === 'hard_goods').length === 0 && (
                      <EmptyState icon={Package} message="No hard goods found for this period" />
                    )}
                  </div>
                )}
              </CardContent>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <Card>
              ...
              <CardContent>
                {suggestedLoading ? <LoadingSkeleton /> : (
                  <div className="space-y-4 h-[240px] overflow-y-auto scrollbar-thin scrollbar-thumb-gray-300 hover:scrollbar-thumb-gray-400">
                    {suggestedProducts
                      .filter((p: any) => p.productType === 'apparel')
                      .map((product: any, index: number) => (
                        <SuggestedCard key={product.id} product={product} index={index} onNavigate={navigateToProducts} />
                      ))}
                    {suggestedProducts.filter((p: any) => p.productType === 'apparel').length === 0 && (
                      <EmptyState icon={Lightbulb} message="No suggested apparel items" />
                    )}
                  </div>
                )}
              </CardContent>
              ...
              <CardContent>
                {suggestedLoading ? <LoadingSkeleton /> : (
                  <div className="space-y-4 h-[240px] overflow-y-auto scrollbar-thin scrollbar-thumb-gray-300 hover:scrollbar-thumb-gray-400">
                    ...
                    {suggestedProducts.filter((p: any) => p.productType === 'hard_goods').length === 0 && (
                      <EmptyState icon={Lightbulb} message="No suggested hard goods" />
                    )}
                  </div>
                )}
              </CardContent>
            ...
            <CardContent>
              {isLoading ? <LoadingSkeleton /> : (
                <div className="space-y-4 h-[240px] overflow-y-auto scrollbar-thin scrollbar-thumb-gray-300 hover:scrollbar-thumb-gray-400">
                  ...
                  {products.length === 0 && (
                    <EmptyState icon={Package} message={`No ${selectedProductType === 'apparel' ? 'apparel' : 'hard goods'} found for this period`} />
                  )}
                </div>
              )}
            </CardContent>
              ...
              {suggestedLoading ? <LoadingSkeleton /> : (
                <div className="space-y-4 h-[240px] overflow-y-auto scrollbar-thin scrollbar-thumb-gray-300 hover:scrollbar-thumb-gray-400">
                  ...
                  {suggestedProducts.length === 0 && (
                    <EmptyState icon={Lightbulb} message={`No suggested ${selectedProductType === 'apparel' ? 'apparel' : 'hard goods'} items`} />
                  )}
                </div>
              )}
            </CardContent>
