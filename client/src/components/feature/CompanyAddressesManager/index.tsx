...
} from "@/services/company-addresses";
...
const emptyForm: CreateCompanyAddress = {
  addressName: "",
  companyNameOnDocs: "",
  street: "",
  street2: "",
...
export function CompanyAddressesManager({ companyId, companyName }: CompanyAddressesManagerProps) {
  const { data: addresses = [], isLoading } = useCompanyAddresses(companyId);
  const createMutation = useCreateCompanyAddress(companyId);
  const updateMutation = useUpdateCompanyAddress(companyId);
  const deleteMutation = useDeleteCompanyAddress(companyId);

  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [editingAddress, setEditingAddress] = useState<CompanyAddress | null>(null);
  const [formData, setFormData] = useState<CreateCompanyAddress>(emptyForm);
  ...
  const openEdit = (address: CompanyAddress) => {
  ...
  const updateField = (field: keyof CreateCompanyAddress, value: any) => {
    setFormData((prev) => ({ ...prev, [field]: value }));
  };

  const formatAddress = (addr: CompanyAddress) => {
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="max-w-5xl">
          <DialogHeader>
            ...
            <DialogDescription>
              {editingAddress
                ? "Update this address for " + companyName
                : "Add a new address for " + companyName}
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="text-sm font-medium">Company Name on Documents</label>
              <Input
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Override the company name that appears on external documents for this address
              </p>
            </div>
